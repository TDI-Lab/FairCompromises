...
class Candidate:
    def __init__(self,
        ...
        self.category = None
        self.district = None
        keys = ['score','latitude','longitude','target','category','categories','district']
        for key in keys:
            if key in proj.keys():
                if key == 'score':
                    self.score = proj[key]
                elif key == 'latitude':
                    self.latitude = proj[key]
                elif key == 'longitude':
                    self.longitude = proj[key]
                elif key == 'target':
                    self.target = proj[key]
                elif key == 'category':
                    self.category = proj[key]
                elif key == 'district':
                    self.district = proj[key]
                else:
                    self.category = proj[key]
        # print(self.score, self.latitude, self.longitude, self.target, self.target, self.category, self.district)
            ...
            with open(filename, 'r', newline='', encoding="utf-8") as csvfile:
                ...
                for i, row in enumerate(reader):
                    ...
                    if str(row[0]).strip().lower() in ["meta", "projects", "votes"]:
                        ...
                        print(project)
                        c_id = row[0]
                        c = Candidate(c_id, int(project["cost"]), project["name"], proj=project, subunit=subunit)
                        self.profile[c] = {}
